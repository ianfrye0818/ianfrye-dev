...
>(({ className, ...props }, ref) => (
  <DrawerPrimitive.Overlay
    className={cn('fixed inset-0 z-50 bg-gray-900/10 opacity-100 backdrop-blur-sm', className)}
    {...props}
    ref={ref}
  />
));
...
const drawerVariants = cva(
  'fixed z-50 shadow-2xl bg-card ring-1 ring-black/10 transition-all ease-in-out duration-100',
  {
    variants: {
      side: {
        right:
